...
from models.lib_mamba.vmambanew import SS2D
...
class Block(nn.Module):
    def __init__(self, c, use_bnck="SE", e=0.5):
        ...
        if use_bnck == "BMamba":
            self.block = BottleneckMamba(c, c, e=e)
        elif use_bnck == "SE":
...
class InceptionMambaBlock(nn.Module):
    ...
    Args:
        ...
        token_mixer (nn.Module): Module for token mixing operations. Default: nn.Identity.
        mlp_layer (nn.Module): Module for MLP operations. Default: ConvMlp.
        mlp_ratio (float): Ratio for hidden dimension in MLP relative to input dim. Default: 4.
        act_layer (nn.Module): Activation layer. Default: nn.GELU.
        ls_init_value (float): Initial value for layer scaling. Set to 0 to disable. Default: 1e-6.
        drop_path (float): Drop path rate for stochastic depth regularization. Default: 0.
        use_bnck (str): Type of normalization/activation to use in token mixer. Default: "SE".
        use_dwconv3x3 (bool): Whether to use depth-wise 3x3 convolution in MLP. Default: False.
    """
...
class InceptionMamba(nn.Module):
    r"""
    Args:
        arch (str): InceptionMamba architecture choosing from 'tiny', 'small' and 'base'. Defaults to 'tiny'
        in_chans (int): Number of input image channels. Default: 3
        num_classes (int): Number of classes for classification head. Default: 1000
        act_layer: Activation function for MLP. Default: nn.GELU
        mlp_ratios (int or tuple(int)): MLP ratios. Default: (4, 4, 4, 4)
...
default_cfgs = dict(
    inceptionmamba_tiny=_cfg(
        url='https://github.com/Wake1021/InceptionMamba/releases/download/cls/inceptionmamba_tiny.pth',
    ),
    inceptionmamba_small=_cfg(
        url='https://github.com/Wake1021/InceptionMamba/releases/download/cls/inceptionmamba_small.pth',
    ),
    inceptionmamba_base=_cfg(
        url='https://github.com/Wake1021/InceptionMamba/releases/download/cls/inceptionmamba_base.pth',
    ),
)

@register_model
def inceptionmamba_tiny(pretrained=False, **kwargs):
    model = InceptionMamba(arch='tiny',**kwargs)
    model.default_cfg = default_cfgs['inceptionmamba_tiny']
    if pretrained:
...
@register_model
def inceptionmamba_small(pretrained=False, **kwargs):
    model = InceptionMamba(arch='small', **kwargs)
    model.default_cfg = default_cfgs['inceptionmamba_small']
    if pretrained:
        state_dict = torch.hub.load_state_dict_from_url(
            url=model.default_cfg['url'], map_location="cpu", check_hash=True)
        model.load_state_dict(state_dict)
...
@register_model
def inceptionmamba_base(pretrained=False, **kwargs):
    model = InceptionMamba(arch='base', **kwargs)
    model.default_cfg = default_cfgs['inceptionmamba_base']
    if pretrained:
        state_dict = torch.hub.load_state_dict_from_url(
            url=model.default_cfg['url'], map_location="cpu", check_hash=True)
        model.load_state_dict(state_dict)
...
if __name__ == '__main__':
    model = inceptionmamba_tiny(pretrained=False)
